...
class CouponDetail extends StatefulWidget {
  ...
  const CouponDetail({Key key, this.id, this.focushour, this.uuid, this.fromprofile, this.status, this.brand}) : super(key: key);
  @override
...
class _CouponDetail extends State<CouponDetail> {
  ...
  Widget build(BuildContext context) {
    Stream stream;
    !widget.fromprofile?stream = FirebaseFirestore.instance
        .collection('coupon').doc(widget.id)
        .snapshots():
    ...
    return
      Scaffold(
          ...
          appBar:new AppBar(backgroundColor: Colors.white,   iconTheme: IconThemeData(
              color: Colors.black,
          ), elevation: 0,           actions: <Widget>[
            new
            GestureDetector(
              onTap: () => Share.share('Sto riscattando questo coupon su Sloff!', subject: 'Coupon di '+widget.brand),
              child:
              IconButton(
            ...
            children: <Widget>[
                  ...
                  builder: (context, snapshot) {
                    if (!snapshot.hasData) {
                      ...
                      return ListView.builder(
                        ...
                        shrinkWrap: true,
                        itemBuilder: (context, index) => buildItem(context, snapshot.data, widget.focushour, widget.id, widget.status),
                        itemCount: 1,
                      );
              ...
              Container(
                ...
                Text('Descrizione lunga',
                    style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold,  fontSize: 15, letterSpacing: -0.39,),
                )
              ...
              Container(
                  padding: EdgeInsets.all(15),
                  child:
                  Text('Descrizione lunga Descrizione lunga Descrizione lunga Descrizione lunga Descrizione lunga '
...
/*
                ...
                children: [
                  GestureDetector(
                      onTap: () async{
                        ...
                        Firestore.instance
                            .collection('users_coupon')
                            .document(uuid).collection(uuid).doc(postID)
                            .updateData({
                          "visible": false
  ...
  Widget buildItem(BuildContext context, DocumentSnapshot document, int focushour, String id, int status) {
    return Container(
        color: Colors.white,
        child: Hero(
            tag: id,
            child:CouponTaken(status: status, title: document['title'], text: document['description'],
                brand: document['brand'], //!document['group']?'Singolo':'Gruppo',
                total: document['total_focus'],
            totalnumber: document['total_coupon'], elapsed: document['elapsed_time'], id: document.id,
            userfocus: focushour)
    ));
                  ...
                  children: [
                        ...
                        Container(
                            ...
                            Column(children: [
                              ...
                              Column(children: [
                                ...
                                Text((userfocus/total*100).toString()+'%', style: TextStyle(color: Colors.black, fontSize: 14, fontWeight: FontWeight.bold),),
                                Container(height: 5,
                                ),
                    ...
                    status>3?Positioned(
                      ...
                      Container(
                          ...
                          decoration: BoxDecoration(
                            color: Color(0xffFF0F5F),
                            borderRadius: BorderRadius.only(topLeft: Radius.circular(8.0), bottomRight: Radius.circular(5.0)),
                          ),
                          //child:
                          // color: Colors.pink,
                          //new Flexible(
                      ...
                      Container(
                        //margin: EdgeInsets.only(left: 15),
                          ...
                          decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.only(topRight: Radius.circular(8.0), bottomLeft: Radius.circular(5.0)),
                          ),
                          //child:
                          // color: Colors.pink,
                          // new Flexible(
                          child: Text(
                            status<3?"Mancano "+(total-userfocus).toString()+" ore\ndi focus":"E' ora di\nriscattarlo",
                            textAlign: TextAlign.center,
                            style: TextStyle(
                              color: Colors.white,
                              fontWeight: FontWeight.w400,
