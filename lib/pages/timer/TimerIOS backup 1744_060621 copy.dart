  ...
  void didChangeAppLifecycleState(AppLifecycleState state) async {
    ...
    switch (state) {
      case AppLifecycleState.resumed:
        ...
        if (warning2 && paused > 1) {
          if (DateTime.now().difference(warningTime).inSeconds > 11) {
            stop();
            setState(() {
              showFocusLost = true;
            });
        ...
        if (!prefs.getBool("lockscreen") &&
            DateTime.now().difference(timeExited).inSeconds > 9) {
          stop();
          setState(() {
            showFocusLost = true;
          });
        } /*  else if (prefs.getBool("lockscreen") && paused > 1) {
            stop();
            setState(() {
              showFocusLost = true;
            });
          } */ /* else if (warning2 &&
              DateTime.now().difference(warning2Time).inSeconds > 9) {
            stop();
            setState(() {
              showFocusLost = true;
            });
      ...
      case AppLifecycleState.paused:
        ...
        print("PAUSA $paused");
        /* if (paused > 1) {
          if (istarted && !prefs.getBool("lockscreen")) {
            _warningNotificationS(DateTime.now().add(Duration(seconds: 2)));
            _lostFocusNotificationS(DateTime.now().add(Duration(seconds: 10)));
    ...
    _timer = new asy.Timer.periodic(
      ...
      (asy.Timer timer) => setState(
        () async {
          ...
          //prefs.setBool("lockscreen", await isLockScreen());

          print("COUNT $timerCount, LOCK " +
              prefs.getBool("lockscreen").toString() +
              ", WARN $warning2, PAUSED $paused");

          if (!warning2 && timerCount > 1 && !await isLockScreen()){
            print("GOT YA!");
            _warningNotificationS(DateTime.now().add(Duration(seconds: 1)));
            _lostFocusNotificationS(DateTime.now().add(Duration(seconds: 12)));
            warningTime = DateTime.now().add(Duration(seconds: 2));
            warning2 = true;
          } */
