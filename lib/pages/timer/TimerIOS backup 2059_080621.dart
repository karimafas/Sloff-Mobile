    ...
    switch (state) {
      case AppLifecycleState.resumed:
        if (!prefs.getBool("lockscreen") &&
            DateTime.now().difference(timeExited).inSeconds > 10) {
          stop();
          setState(() {
            showFocusLost = true;
          });
        } else if (prefs.getBool("lockscreen") && paused > 3) {
          stop();
          setState(() {
            showFocusLost = true;
          });
          ...
          DateTime startedtime =
              new DateTime.fromMillisecondsSinceEpoch(started);

          int timeToWrite = prefs.getInt("timeToRecord");

          if (prefs.getBool("record") && prefs.getBool("lockscreen") && canWrite()) {
            writeDb(timeToWrite);
