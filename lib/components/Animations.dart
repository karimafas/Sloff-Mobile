...
class SlideYFadeInOut extends StatelessWidget {
  final double delay;
  final bool open;
  final Widget child;

  SlideYFadeInOut(this.delay, this.open, this.child);

  @override
  Widget build(BuildContext context) {
    final tween = MultiTrackTween([
      Track("opacity")
          .add(Duration(milliseconds: open ? 500 : 150), Tween(begin: open ? 0.0 : 1.0, end: open ? 1.0 : 0.0)),
      Track("translateY").add(
          Duration(milliseconds: open ? 500 : 150), Tween(begin: open ? 15.0 : 0.0, end: open ? 0.0 : 15.0),
          curve: Curves.easeOut)
    ]);
